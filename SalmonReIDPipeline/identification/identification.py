...
from training_testing_and_analysis import collate_fn, create_results_folder, analyze_data_tsne, train_reid_model, test_reid_model, get_resnet101_withclslayer
...
def dataloaders(bodypart, hyperparameters):
...
def train_model(model, train_loader, validation_loader, hyperparameters, device, folder):
    ...
    train_reid_model(model, criterion, optimizer, scheduler, train_loader, validation_loader, EPOCHS, hyperparameters, device, folder)
...
def roc_models(bodyparts, hyperparameters, modelpaths, APPROACH):
    ...
    for i, bp in enumerate(bodyparts):
        ...
        predictions, targets, probabilities, corrects = test_reid_model(model_withcls, test_loader, device)
...
def plot_accuracies(bodyparts, hyperparameters, modelpaths, APPROACH):
    ...
    for i, bp in enumerate(bodyparts):
        ...
        predictions, targets, probabilities, corrects = test_reid_model(model_withcls, test_loader, device, bp)
...
def main():
    ...
    for i in range(5):
        hyperparameters['bodypart'] = bodyparts[i]
        BODYPART = hyperparameters['bodypart']
        MODELPATH = modelpaths[i]
        
        _, _, test_loader, device = dataloaders(BODYPART, hyperparameters)
        ...
        if bodyparts_titles[i] == 'Thorax':
            predictions, targets, _, _ = test_reid_model(model_withcls, test_loader, device, bodyparts_titles[i])
            
            tot_predictions[i] = predictions
            tot_targets[i] = targets
        
    try:
        with open('predictions_targets_reid_test.json', 'w') as f:
