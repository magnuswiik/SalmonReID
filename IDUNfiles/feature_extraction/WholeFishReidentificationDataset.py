...
class WholeFishReidentificationDataset(Dataset):
    def __init__(self, data_path, landmark, transform=None):
        ...
        for fish in os.listdir(data_path):
            if not fish.startswith('.'):
                    for file in sorted(os.listdir(os.path.join(data_path, fish))):
    ...
    def __getitem__(self, idx):
        ...
        anchor_img_path = os.path.join(self.data_path, self.images[idx][:self.images[idx].find("_", 0, 7)], self.images[idx])
        ...
        positive_img_path = os.path.join(self.data_path,self.images[positive_idx][:self.images[positive_idx].find("_", 0, 7)], self.images[positive_idx])
        ...
        negative_img_path = os.path.join(self.data_path, self.images[negative_idx][:self.images[negative_idx].find("_", 0, 7)], self.images[negative_idx])
